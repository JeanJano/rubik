...
UDSTwoCoord UDSTwoCoord::move(const GOneMove& move) const {
    UDSTwoCoord newCoord = *this;
    ...
    switch (move) {
        ...
        case GOneMove::L2:
            ...
            break;
        default:
            std::cerr << "Fatal error in UDSTwoCoord::GOneMove: unsupported GOneMove." << std::endl;
            break;
    }
...
UDSTwoCoord UDSTwoCoord::nextExplicitCoord(){
    UDSTwoCoord next = UDSTwoCoord::fromNextOrderDiag();

    // std::cout << "next :" << std::endl;
    // next.printExplicitUDSTwoCoord();
    return next;
}

UDSTwoCoord UDSTwoCoord::fromNextOrderDiag() {
    UDSTwoCoord ret = *this;
    std::vector<int> avalaibleValues = {0,1,2,3};
        // print_first(avalaibleValues, 8);

        // std::cout << "vista de ret: antes " << N << std::endl;
        // ret.printExplicitUDSTwoCoord();
...
void UDSTwoCoord::defineOrderDiag(){
...
void UDSTwoCoord::oneStep(int BigIndex) {
...
void UDSTwoCoord::defineNextOrderDiag(){
...
void UDSTwoCoord::printMoveTable(){
    UDSTwoCoord state;
    for (int i = 0; i < 24; ++i){
        ...
        // state.printExplicitUDSTwoCoord();
...
void UDSTwoCoord::moveTableToFile() {
    std::error_code ec;
    ...
    std::string filepath = movesFoldername + UDSTwoTableFilename;
    ...
    out.close();
    std::cout << "UDslice fase two move table done!" << std::endl;
}